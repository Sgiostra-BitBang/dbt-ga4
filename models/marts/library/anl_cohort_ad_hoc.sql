...
with initial as (
    select
        ...
        concat( extract( year from  first_seen_dt ), "-" ,extract( month from  first_seen_dt ) 
            {% if var('cohort_period', 'month') == 'week'  %}
...
cohorts as (
    select   -- adding user_key here and removing the sum and changing the THEN to then +=1 would create a more permanent table that you could sum in your final reports
        {% for num in range( var( 'num_cohorts', 6) ) %}
            sum(case when date_diff(initial.first_seen_dt, {{ var('cohort_column_date','session_start_date')  }}, {{ var('cohort_period','month') }}  ) = {{ num }} then 1 end) as {{ var('cohort_period','month') }}_{{num}},
        {% endfor %}
        initial.initial_cohort
    from  {{ ref( var( 'cohort_table', 'fct_ga4__sessions')  ) }}
    right join initial using(user_key)
    group by initial.initial_cohort
)
select
    *
from cohorts